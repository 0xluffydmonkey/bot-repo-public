...
Este guia cobre tudo o que um operador precisa para rodar o bot com segurança em produção:
- quais são as superfícies de controle
- como usar o dashboard
- como usar o bot de controle do Telegram
...
O bot tem duas superfícies de controle para o operador. Ambas leem e escrevem no mesmo estado compartilhado do backend — são equivalentes:
...
| Reduzir parcialmente uma posição | `POST /api/reduce` |

> O dashboard tem controles diretos para pause, resume, AT e close. Para operações manuais de trading, use a API REST ou o bot de controle do Telegram.
...
- valida suporte ao ativo
- valida relação R:R
- verifica colateral livre
- verifica max de posições
- verifica max de exposição

Se o risk manager rejeitar, o bot informa o motivo.
...
O bot valida step size e quantidade mínima antes de executar.
...
**O que faz:** quando desativado, todos os sinais recebidos são descartados silenciosamente antes de qualquer processamento — antes do parse, deduplicação e verificações de risco. Nenhuma entrada `signalIgnored` é criada. Posições abertas não são afetadas.

**Quando usar:**
- recebendo uma enxurrada de sinais inválidos do canal monitorado
- durante gerenciamento manual de posições quando você não quer novas entradas automáticas
...
- Socket.IO: `cmd:intake` com `{ enabled: false }`
...
Os sinais passam por estes filtros em sequência:
...
## Comportamento do Operador em Modo Paper

Em modo paper, todas as superfícies do operador funcionam de forma idêntica ao modo live:
- abertura, fechamento, redução e TP/SL manual funcionam normalmente
- o dashboard mostra posições e PnL reais do paper engine
- o Telegram mostra cards de posição e alertas reais
- os controles operacionais (pause, intake, AT) funcionam normalmente

A diferença: **nenhuma transação blockchain real é enviada**. O paper engine simula a execução das posições em memória.

Posições e saldo em modo paper são resetados quando o bot reinicia.

Mensagens em modo paper são rotuladas com `🧪 PAPER` no Telegram para evitar confusão com operações reais.

Veja [modo-paper.md](modo-paper.md) para o comportamento detalhado do paper vs live.
