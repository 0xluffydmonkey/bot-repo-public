...
- `closeManualTrade()` em [backend/src/trading/ManualTradeService.js](../../backend/src/trading/ManualTradeService.js)
- `closeAllManualTrades()` em [backend/src/trading/ManualTradeService.js](../../backend/src/trading/ManualTradeService.js)
...
- fluxos REST e Socket.IO do dashboard em [backend/src/web/server.js](../../backend/src/web/server.js)
- callbacks de close individual no Telegram em [backend/src/telegram/handlers/callbacks.js](../../backend/src/telegram/handlers/callbacks.js)
...
- handlers `cmd:close` e `cmd:close_all` em [backend/src/index.js](../../backend/src/index.js)
- trailing/system close em [backend/src/trading/position-management/PositionManager.js](../../backend/src/trading/position-management/PositionManager.js)
...
- o helper manual direto `closeAllManualTrades()` ainda permite fallback hoje por compatibilidade retroativa
...
- comportamento estrito em [backend/src/index.js](../../backend/src/index.js)
- fallback ainda permitido no helper manual em [backend/src/trading/ManualTradeService.js](../../backend/src/trading/ManualTradeService.js)
- o `close_all` do Telegram atualmente usa `state.positions[0]?.venue` sob a suposição prática atual de uma única venue por vez
...
- resolver compartilhado canônico para seleção de venue no close
- suporta:
  - `explicit`
  - `position`
  - `active_fallback`
  - `unresolved` quando o fallback está desabilitado
...
- atualmente eles seguem a política estrita do command-bus
...
- `close_all` ainda reflete a suposição prática atual de uma única venue por vez
...
Se você só precisa do resumo de alto nível:

- veja [README.pt-BR.md](../../README.pt-BR.md)
- veja [PROJECT_STATE.md](../../PROJECT_STATE.md)

Se você precisa do detalhe de implementação:

- este documento é a referência canônica
